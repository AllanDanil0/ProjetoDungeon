...
const RubraWeaponArt=(()=>{
 ...
 function iconHtml(id){if(!icons[id]){const c=document.createElement('canvas');const size=RubraConfig.profane.weapons.concat('verdict','absolute','prism').includes(id)?96:32;c.width=c.height=size;item(c.getContext('2d'),id,size/2,size/2,0,size/32);icons[id]=c.toDataURL();}return `<img class="weapon-icon" alt="" src="${icons[id]}">`;}
 ...
 function effect(q,e,time){if(RubraIcePolish.prism(q,e,time)||RubraSanctuary.absoluteEffect(q,e,time)||RubraProfane.effectArt(q,e,time))return;q.save();
