...
const RubraAudio=(()=>{
 ...
 class Engine{
  ...
  tick(){if(!this.options.music||!scores[this.scene]){this.next=this.ctx.currentTime+.03;return;}const c=this.ctx,score=scores[this.scene],beat=60/score.bpm/2;if(this.next<c.currentTime-.1)this.next=c.currentTime+.02;for(let scheduled=0;this.next<c.currentTime+.12&&scheduled<3;scheduled++){const i=this.step++,t=this.next,n=score.notes[i%16];this.tone(n,t,beat*.85,'triangle',.12);if(i%4===0)this.tone(score.bass[Math.floor(i/8)%4],t,beat*3.5,'sine',.19);if(i%2===0)this.tone(33,t,.1,'sine',.13,'music',-15);else this.tone(91,t,.035,'triangle',.018);if(this.scene==='boss'&&i%2===0)this.tone(n-12,t,beat*.6,'sawtooth',.035);this.next+=beat;}}
