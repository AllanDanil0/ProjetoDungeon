...
Validação local: check, 36 regras, 62 integração, 25 menus, 11 Profano, 12 refinamento, 20 resultados, 16 atualização anterior, 13 inicialização, 14 gelo/interface. Build 1.11 gerado e verify:package aprovado; inicialização e gelo repetidos no ASAR. Portátil real passou em 13 verificações, com campanha isolada reaberta e cliques reais. Janela nativa inicia em tela cheia e F11 alterna nos dois sentidos (3 verificações no main.js empacotado). A falha inicial do teste era causada pela função global screen do jogo ocultando Screen; medição foi corrigida. Eventos CDP não acionam before-input-event, portanto o F11 é testado com sendInputEvent nativo. Capturas e ícone extraído do EXE inspecionados. Publicação ainda pendente nesta etapa. Saves reais não foram alterados.
