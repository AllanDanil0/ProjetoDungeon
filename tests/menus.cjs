...
try{
 ...
 await run("const previous=save;save=K.defaults();armoryScreen();if($('armoryCards').children.length!==Object.keys(C.weapons).length||document.querySelectorAll('.armory-card[data-unlocked=true]').length!==1||document.querySelectorAll('.armory-card .weapon-icon').length!==Object.keys(C.weapons).length)throw Error('Arsenal incompleto');save=previous;menu();$('armoryButton').click();if(state!=='armory')throw Error('Arsenal inacessível');$('armoryBack').click();if(state!=='menu')throw Error('Voltar do Arsenal');");checks.push('Arsenal lists locked/unlocked weapons with icons and live navigation');
 const screens={menu:'menu()',characters:'charactersScreen()',maps:'mapsScreen()',options:'menu();optionsScreen()',help:"menu();$('menuControls').click()",laboratory:'laboratoryScreen()',armory:'armoryScreen()'};
 for(const [width,height]of [[1360,900],[960,640],[420,850]]){
  ...
  for(const [id,open]of Object.entries(screens)){
   ...
   if(id==='options')assert.ok(await run("Array.from($('options').querySelectorAll('.dialog>p')).every(p=>{const a=p.getBoundingClientRect(),b=$('options').querySelector('.preferences-panel').getBoundingClientRect();return Math.abs((a.left+a.right-b.left-b.right)/2)<2&&getComputedStyle(p).textAlign==='center';})"),'Options help text must share panel center');
 ...
 await run("save=K.defaults();save.characters=Object.keys(C.characters);save.maps=Object.keys(C.maps);save.weapons=Object.keys(C.weapons);save.options.testArsenal=false;for(const hero of Object.keys(C.characters))for(const arena of Object.keys(C.maps)){save.selectedCharacter=hero;save.selectedMap=arena;begin();if(run.weapons[0].id!==C.characters[hero].weapon)throw Error(hero+' em '+arena);state='paused';}menu();");checks.push('Every hero starts every map with their own signature weapon');
 await run("const context=new OfflineAudioContext(1,22050,22050),engine=new RubraAudio.Engine(context),notes=[];engine.setOptions({music:true,sound:false,musicVolume:.4,effectsVolume:0});engine.setScene('ice');engine.step=128;engine.tone=(note)=>notes.push(note);engine.tick();if(notes[0]!==RubraAudio.scores.iceAurora.notes[0])throw Error('Nova música não iniciou');engine.dispose();");checks.push('Ice music switches to Aurora after its first phrase cycle');
