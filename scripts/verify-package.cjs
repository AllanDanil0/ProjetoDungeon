...
const shipped=fs.readdirSync('src').map(f=>'src/'+f).concat(fs.readdirSync('assets/visual-update').map(f=>'assets/visual-update/'+f),fs.readdirSync('assets/expansion').map(f=>'assets/expansion/'+f),['index.html','main.js']);
