...
const shipped=fs.readdirSync('src').map(f=>'src/'+f).concat(fs.readdirSync('assets/visual-update').map(f=>'assets/visual-update/'+f),fs.readdirSync('assets/expansion').map(f=>'assets/expansion/'+f),fs.readdirSync('assets/ice').map(f=>'assets/ice/'+f),fs.readdirSync('assets/ornate').map(f=>'assets/ornate/'+f),fs.readdirSync('assets/profane').map(f=>'assets/profane/'+f),['index.html','main.js']);
